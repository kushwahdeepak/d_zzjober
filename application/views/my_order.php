...
<?php
    ...
    $user_invoice_list = $this->user_helper->getUserInvoiceProduct($user_id);
?>
            ...
            <div class="container holder accordion-panel-group panel">
                ...
                <?php if (!empty($user_invoice_list)) {
                    foreach ($user_invoice_list as $invoice) {
                        $invoice_product_list = $this->user_helper->getInvoiceProduct($invoice->invoice_id);
                        ...
                        <h4>
                            <a class="accordion-panel-title" data-toggle="collapse" data-parent="#accordion-panel"
                               href="#collapse<?php echo $invoice->invoice_id ?>">
                                <div class="row">
                                    <div class="col-sm-4">
                                        <h5 style="color: black"><b>Order Id:</b> #00<?php echo $invoice->invoice_id; ?></h5>
                                    </div>
                                    <div class="col-sm-4">
                                    ...
                                    <div class="col-sm-4">
                                        <h5 style="color: black"><b>Address:&nbsp;</b><?php echo $invoice->address; ?></h5>
                                    </div>
                                </div>
                            </a>
                        </h4>
                            ...
                            <table id="example01" class="table cart-table">
                                ...
                                <?php if (!empty($invoice_product_list)) {
                                    foreach ($invoice_product_list as $invoice_product) {
                                        ...
                                        $provider_feedback_info = $this->user_helper->getProviderFeedbackDetail($invoice_product->product_invoice_id);
                                         ?>
                                            ...
                                            <td class="product-name-col">
                                                ...
                                                <div class="col-md-7">
                                                    ...
                                                    <ul>
                                                        ...
                                                        <li><b>Status: </b><?php echo $product_invoice_id->client_status; ?></li>
                                            ...
                                            <td class="product-price-col">
                                                <?php
                                                    if ($product_invoice_id->client_status == 'Cancel') {
                                                        ...
                                                        echo '</div>';
                                                        echo '</div>';
                                                    } else if($product_invoice_id->client_status == 'Pending' && $product_invoice_id->provider_status == 'Pending') { ?>
                                                        <a class="product-add-btn product-add-btn-custom"
                                                           onclick="cancelOrder(<?php echo $invoice_product->product_invoice_id; ?>, <?php echo $invoice_product->invoice_id; ?>)"
                                                           href="javascript:void(0);" title="Cancel">Cancel
                                                        </a>
                                                    ...
                                                    <?php } else if(($product_invoice_id->provider_status == 'Complete') && ($product_invoice_id->client_status == 'Pending')) {
                                                        ...
                                                        <a class="product-add-btn product-add-btn-custom"
                                                           onclick="completeOrder(<?php echo $invoice_product->product_invoice_id; ?>)"
                                                           href="javascript:void(0);" title="Mark Complete">Mark Complete</a>

                                                        <a class="product-add-btn product-add-btn-custom"
                                                           onclick="inCompleteService(<?php echo $invoice_product->product_invoice_id; ?>)"
                                                           href="javascript:void(0);" title="Mark Incomplete">Mark Incomplete</a>

                                                    <?php } else if(($product_invoice_id->provider_status == 'Complete') && ($product_invoice_id->client_status == 'Complete')) {
                                                        ...
                                                        echo '<a class="product-add-btn product-add-btn-custom check feedback_button" href="javascript:void(0)" title="Write Review" 
                                                    ...
                                                    } else if(($product_invoice_id->provider_status == 'Complete') && ($product_invoice_id->client_status == 'Reviewed')) {
                                                        echo '<div class="complete_color status_size" style="line-height: 20px;">';
                                                        echo "Order Reviewed";
                                                        echo '</div>';
                                                    } else if($product_invoice_id->provider_status == 'Reviewed') {
                                                        echo '<a class="product-add-btn product-add-btn-custom check" href="javascript:void(0)" title="View Review" onclick="ViewReviewedProvideByClient('.$provider_feedback_info->feedback_id.')">';
                                                        echo 'View Review';
                                                        echo '</a>';
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?php echo site_url('user_controller/customerFeedback'); ?>" name="review_form" method="post">
...
<div class="modal fade" id="cancel_model" role="dialog">
    <div class="modal-dialog">
        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" onclick="removeFromCart(<?php echo $invoice_product->product_invoice_id; ?>)"
                        class="close" data-dismiss="modal">&times;
                </button>
                <h4 class="modal-title" style="color:white">Cancel Order</h4>
            </div>
            <div class="modal-body">
                <form action="<?php echo site_url('user_controller/cancelOrder'); ?>" method="post">
                    <input type="hidden" class="product_invoice_id" name="product_invoice_id" value="">
                    <input type="hidden" class="invoice_id" name="invoice_id" value="">
                    <div class="form-group">
                        <textarea name="reason" class="form-control input-lg min-height" cols="30" rows="6"
                                  placeholder="Reason for cancelling order*" required></textarea>
                    </div>
                    <div class="xs-margin"></div>
                    <div class="form-group">
                        <input type="submit" class="btn btn-custom-5 btn-lg min-width" value="Cancel Order">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
<div class="modal fade" id="incomplete_model" role="dialog">
    <div class="modal-dialog">
        <!-- Modal content-->
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?php echo site_url('user_controller/rejectServiceCompleteStatus'); ?>" method="post">
                    <input type="hidden" class="product_invoice_id" name="product_invoice_id" value="">
                    <div class="form-group"><textarea name="reason" class="form-control input-lg min-height" cols="30" rows="6"
                                  placeholder="Reason for rejecting complete status*" required></textarea>
                    </div>
                    <div class="xs-margin"></div>
                    <div class="form-group">
                        <input type="submit" class="btn btn-custom-5 btn-lg min-width" value="Reject">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
<div class="modal fade" id="feedback_modal_client" role="dialog">
    <div class="modal-dialog">
        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" style="color:white"> Feedback By Client </h4>
            </div>
...
<script type="text/javascript">
    function completeOrder(product_invoice_id)
    {
        $.ajax({
            url: "<?php echo base_url();?>index.php/user_controller/clientCompleteOrder?product_invoice_id="+product_invoice_id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
                location.reload();
                toastr.success('Your order is completed');
            }
        });
    }
</script>
...
<script type="text/javascript">
    function cancelOrder(product_invoice_id, invoice_id)
    {
        $(".product_invoice_id").val(product_invoice_id);
    ...
    function inCompleteService(product_invoice_id)
    {
        $(".product_invoice_id").val(product_invoice_id);

        $("#incomplete_model").modal('show');
    }
</script>


<script type="text/javascript">
    function removeFromCart(product_invoice_id)
    {
        $.ajax({
            url: "<?php echo base_url();?>index.php/user_controller/userNotificationStatus?product_invoice_id="+product_invoice_id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
                location.reload();
                toastr.success('Your product deleted');
            }
        });
    }
</script>
    ...
    function ViewReviewedProvideByClient(feedback_id)
    {
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#feedback_modal_client").modal('show');
            }
        });
